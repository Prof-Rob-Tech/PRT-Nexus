...
from services.extractors.tiktok_connector import TikTokWorker, separar_links
...
class TikTokView(QWidget):
    ...
    def _aplicar_estilos(self):
        self.setStyleSheet("""
            ...
            QLineEdit:focus, QPlainTextEdit:focus, QComboBox:focus { border: 1px solid #FE2C55; }
            QLabel.lbl-box {
                background-color: #252526;
                border: 1px solid #3a3a3a;
                border-radius: 6px;
    ...
    def _montar_interface(self):
        ...
        lbl_titulo = QLabel("Conector TikTok")
        lbl_titulo.setStyleSheet("font-size: 24px; font-weight: bold; color: #ffffff;")
        lbl_sub = QLabel("Vídeo público e perfil público, sem login. Conta fechada e vídeo só para amigos ficam de fora.")
        ...
        gb_captura = QGroupBox("🔗 Links do TikTok")
        ...
        self.btn_baixar.setStyleSheet(
            "background-color: #FE2C55; color: #ffffff; font-weight: bold; padding: 8px; border-radius: 8px; border: none;"
        )
        ...
        self.txt_nome_conteudo.setPlaceholderText("Vazio usa o título do TikTok. Com vários links, cada um fica na própria pasta.")
        ...
        ly_prog = QHBoxLayout()
        self.lbl_status_global = QLabel("Aguardando link do TikTok...")
        ...
        self.pbar_global.setStyleSheet("""
            ...
            QProgressBar::chunk { background-color: #FE2C55; border-radius: 6px; }
        """)
        ...
        lbl_tab = QLabel("📦 Vídeos do TikTok (duplo clique abre a pasta)")
    ...
    def _iniciar_download(self):
        ...
        if not links:
            QMessageBox.warning(self, "Link vazio", "Cole um ou mais links do TikTok, um por linha.")
            return
        self.txt_destino.deselect()
        ...
        self.worker = TikTokWorker("", destino, opcoes=opcoes, urls=links)
    ...
    def _on_concluido(self, sucesso, mensagem):
        ...
        if self.chk_notif.isChecked():
            ...
            QApplication.beep()
        if sucesso:
            QMessageBox.information(self, "TikTok", mensagem)
        else:
            QMessageBox.critical(self, "TikTok", mensagem)
